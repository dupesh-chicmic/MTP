...
/**
 * This is the model class for table "used_cars_model".
 */
class UsedCarsModel extends CActiveRecord
{
	 ...
	 * @return UsedCarsModel the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('id_used_cars', 'required'),
			array('id_used_cars, order', 'numerical', 'integerOnly' => true),
			...
			array('id_used_cars, order, codenumber, maker, vehicle, badge, body, price, years, fuel, yr0, kms0, GRP0, yr1, kms1, GRP1, yr2, kms2, GRP2, yr3, kms3, GRP3, yr4, kms4, GRP4, yr5, kms5, GRP5, yr6, kms6, GRP6, yr7, kms7, GRP7, spec1, spec2, spec3, spec4, spec, intro1, intro2, intro3, intro4, intro5, note1, note2, note3, note4, note5', 'safe', 'on' => 'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'used_cars_cars_model' => array(self::BELONGS_TO, 'UsedCars', 'id'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('id_used_cars', $this->id);
		...
		//$criteria->compare('newprice0', $this->newprice0, true);
	...
	public static function ensureKmsFormat($kms)
	{
		//make sure that the kms value was passed as thousands 
	...
	//fetch data for Archive by make and model page
	public static function fetchCarsData($type = 'cars', $carId, $rangecode, $corecode)
	{
		$table = 'used_cars_model';
		$fildName = 'id_used_cars';
		//table name according to type of request
		if ($type == 'commercial') {
			$table = 'used_com_cars_model';
			$fildName = 'id_used_com_cars';
		}

		$where = " $fildName = " . $carId;

		if ($rangecode) {
			$where .= " AND t.rangecode = " . $rangecode;
		}
		...
		$query = "SELECT * FROM $table t WHERE " . $where;
	...
	public static function getRangeDesc($rangecode, $type = 'cars')
	{

		$table = 'used_cars_ranges';

		if ($type == 'commercial') {
			$table = 'used_comms_ranges';
		}

		$where = ' ';

		if ($rangecode) {
			$where .= " WHERE rangecode = $rangecode ";
		}

		$query = "SELECT rangedesc FROM $table $where limit 1";

		$result = Yii::app()->db->createCommand($query)->queryAll();

		if ($result) {
			$rangedesc = $result[0]['rangedesc'];
		}
