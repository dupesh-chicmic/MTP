...
echo '</div>';
echo '<div>'.$car['maker'].'</div>';
echo '<div>'.$car['vehicle'].'</div>';
echo '<div>'.$car['badge'].'</div>';
...
if(Mobile::isNumber($info['value'])){
        $valueString = '<div class="results emphasize4" style="text-align:center; margin-bottom: -15px; padding-bottom: 20px;">Guide Price <span>&euro;'.Mobile::displayValue($info['value']).'</span></div>';
    }else {
...
echo CHtml::beginForm(Yii::app()->createUrl('mobile/gSelectMakeComms'), 'POST', array('id'=>'commsForResult','autocomplete'=>'off' ));
...
echo CHtml::hiddenField('comms_mark_name', $_POST['comms_mark_name']);
echo CHtml::hiddenField('comms_ranges', $_POST['comms_ranges']);
echo CHtml::hiddenField('comms_model', $_POST['comms_model']);
echo CHtml::hiddenField('comms_fuel', $_POST['comms_fuel']);
echo CHtml::hiddenField('comms_transmission', $_POST['comms_transmission']);
echo CHtml::hiddenField('comms_body', $_POST['comms_body']);
echo CHtml::hiddenField('comms_doors', $_POST['comms_doors']);
echo CHtml::hiddenField('comms_badge', $_POST['comms_badge']);
...
?>
    <div class="results emphasize3 dis-blk">
        ...
        <div class="res_res"><span class="emphasize3 text-left" style="text-align: left;">
            <a onclick="submitForm()" data-role="button" src="images/mobile/go.png" data-shadow="false" data-iconpos="notext" data-theme="none" >Go again!</a></span>
        </div>
...
<script type="text/javascript">
    ...
    function validate(){
        if($('#commsForResult #comms_badge').val()=='') return false;
        if($('#commsForResult #comms_doors').val()=='') return false;
        if($('#commsForResult #comms_transmission').val()=='') return false;
        if($('#commsForResult #comms_fuel').val()=='') return false;
        if($('#commsForResult #comms_model').val()=='') return false;
        if($('#commsForResult #comms_ranges').val()=='') return false;
        if($('#commsForResult #comms_mark_name').val()=='') return false;
        if($('#commsForResult #year').val()=='') return false;
        return true;
    }
    function goBack(){
        window.location.href='<?php echo Yii::app()->createUrl('mobile/gSelectMakeComm');?>';
    }
</script>
<style>
