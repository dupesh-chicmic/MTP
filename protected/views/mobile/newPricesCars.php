...
<div id="dw-control-group"  data-role="controlgroup" data-type="horizontal">
        <a href="<?php echo Yii::app()->createUrl('mobile/newPricesCars').'&page=cars';?>" class="dw-radio-btn ui-btn ui-corner-all <?=($page == 'cars') ? 'ui-btn-active' : '' ?>" >Passenger</a>
        <a href="<?php echo Yii::app()->createUrl('mobile/newPricesCars').'&page=commercial';?>" 
        ...
        foreach ($cars->make as $row) {
            ...
            if(!empty($manufacturerName)){
                ...
                foreach($range_array[$manufacturerName] as $range_name=>$range_val){
                    ...
                    echo '<li class="range_model_name '.$rangeClass.'""><a class="' . $rangeClass . '" href="index.php?r=mobile/newPricesCars&make=' . urlencode((String)$row['file']) . '&rangecode='.urlencode((string)$range_val).'&page='.$page.'">' . $range_name .'</a></li>';
