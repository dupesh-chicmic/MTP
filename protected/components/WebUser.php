 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CWebUser.php 2026 2010-04-07 19:34:29Z alexander.makarow $
...
class WebUser extends CApplicationComponent implements IWebUser
{
	...
	public $loginUrl=array('site/loginIframe');
	/**
	...
	private $_access=array();
        ...
        /**
         * @var Klient|Pracownik model danych użytkownika aktualnie zalogowanego
         */
        private $_model=null;
        public $as_klient_uzytkownik=null;

        /**
         * @var Klient model z danymi klienta zalogowanego
         * lub w imieniu którego pracuje pracownik
         */
        private $_customerModel;

        private $_isMobileOn = false;

        private $_checksStatus = 0;
		private $websiteurl=array();
		function __construct() {
			$this->loginUrl=Yii::app()->params['login_url'];
			//$this->loginUrl
		}
	/**
	 * PHP magic method.
	...
	public function init()
	{
		...
		$this->updateFlash();
                 ...
                 if( $this->hasState('lang')){
                        //Yii::app()->language = $this->getState('lang');
                    $this->setLanguage($this->getState('lang'));
                }
	}
	...
	public function login($identity,$duration=0)
	{
		$this->changeIdentity($identity->getId(),$identity->getName(),$identity->getPersistentStates());
                $this->setState('last_request_time', time());
	...
	{
            //usuń identyfikator sesji
            if(Yii::app()->user->getModel())
            {
                Yii::app()->user->getModel()->uzytkownik->sid=null;
                Yii::app()->user->getModel()->uzytkownik->saveAttributes(array('sid'));
            }

            //FtpClientAccount::model()->deleteAll('customer_id='.$this->getKlientUserID());
	 ...
	 */
        ...
        /*TODO: tę właściwość trzeba zbuforować albo przenieść do funkcji weryfikującej sesję
         *
         */
	public function getIsGuest()
	{
            /** @TODO olek: to jest jakieś dziwne, gdzie jest hasOutdatedSession? **/
            if($this->hasState('last_request_time'))
                if(time()-$this->getState('last_request_time')>Yii::app()->getSession()->getTimeout())
                            //$this->logout();
                        $this->setState('__id', null);
                else
                    $this->setState('last_request_time', time());
        ...
        public function hasOutdatedSession()
        {
            if($this->hasState('last_request_time'))
                return $this->getIdleTime()>Yii::app()->getSession()->getTimeout();
            return false;
        }

        public function getIdleTime()
        {
            return time()-$this->getState('last_request_time');
        }
	...
	public function checkAccess($operation,$params=array(),$allowCaching=true)
	{
        ...
        {
            if($this->getState('employee') || $this->getState('admin')) {
                    return true;
            }
            return false;
        }
        ...
        public function getModel()
        {
            if(!$this->_model)
            {
                ...
                {
                   $this->_model=Pracownik::model()->with('uzytkownik')->findByPk($this->id);
                }
                elseif($this->getIsCustomer())
                    {
                        //$this->_model=Klient::model()->with('uzytkownik')->findByPk($this->id); // Mariusz
                    }
                    else return null;
            }
        ...
        {
            //return $this->getState('agency', true)?true:false;

            if ($this->getIsCustomer()){

                $model = $this->getmodel();
                //echo 'jest customer w gia web user'.$model->getIsAgency();
                return $model->getIsAgency();
            }else {
                if (($this->getState('as_klient_uzytkownik'))!=null){
                    //$user = Uzytkownik::model()->findbyPk($this->getState('as_klient_uzytkownik'));
                    $klient = Klient::model()->with('uzytkownik')->findByPk($this->getState('as_klient_uzytkownik'));
                    return $klient->getIsAgency();
                }
                return false;
            }
  ...
  public function getCompany(){
         ...
         public function getIsCompany(){
            if ($this->getIsCustomer()){
                $model = $this->getmodel();
                return $model->getIsCompany();
            }else {
                if (($this->getState('as_klient_uzytkownik'))!=null){
                    //$user = Uzytkownik::model()->findbyPk($this->getState('as_klient_uzytkownik'));
                    $klient = Klient::model()->with('uzytkownik')->findByPk($this->getState('as_klient_uzytkownik'));
                    return $klient->getIsCompany();
                }
                return false;
            }
        ...
        public function setIsAgency($value)
        {
        ...
        /**
         *
         * @param string $typ_adresu - nazwa słownikowa typu adresu do wczytania
         * @return KlientAdres
         */
        public function pobierzAdres($typ_adresu){
            if ($this->getIsCustomer()){
                $model = $this->getmodel();
                return $model->pobierzAdres($typ_adresu);
            }else {
                if (($this->getState('as_klient_uzytkownik'))!=null){
                    //$user = Uzytkownik::model()->findbyPk($this->getState('as_klient_uzytkownik'));
                    $klient = Klient::model()->with('uzytkownik')->findByPk($this->getState('as_klient_uzytkownik'));
                    return $klient->pobierzAdres($typ_adresu);
                }
                return false;
            }
        ...
        public function getKlientUserID(){
            if ($this->getIsCustomer()){
                //$model = $this->getmodel();
                return $this->getId();
            }else {
        ...
        public function getCustomerModel()
        {
            ...
            if(($this->getState('as_klient_uzytkownik'))!=null)
            {
                $this->_customerModel=Klient::model()->with('uzytkownik')->findByPk($this->getState('as_klient_uzytkownik'));
                return $this->_customerModel;
            }
        ...
        public function getTabMenuIdx($menuID) {
            if($this->hasState('tabMenuCurrent')) {
                $s=$this->getState('tabMenuCurrent');
                $menuID=(int)$menuID;
                if(isset($s[$menuID])) {
                    return $s[$menuID];
                }
        ...
        public function setTabMenuIdx($menuID, $idx) {
            if($this->hasState('tabMenuCurrent')) {
               $s=$this->getState('tabMenuCurrent');
            }

            $s[$menuID]=(int)$idx;
            $this->setState('tabMenuCurrent', $s);
        }

        public function getShowOrders() {
            if($this->hasState('showOrders')) {
               return $this->getState('showOrders');
            }

            return 1;
        }

        public function setShowOrders($value=1) {
             $this->setState('showOrders', (int)$value);
        }

       public function setLanguage($lang){
            Yii::app()->language = $lang;
            $this->setState('lang',Yii::app()->language);
        }

        public function getIsMobileOn()
        {
            $this->_isMobileOn = Mobile::getUserIsMobileOn();
            return $this->_isMobileOn;
        }

        public function getIsCheckOnly()
        {
            $this->_checksStatus = Uzytkownik::getUserChecksStatus();
            return ($this->_checksStatus == 1) ? true : false;
        }
